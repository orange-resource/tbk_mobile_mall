...
public class AliyunOssImpl extends ServiceImpl<AliyunOssMapper, AliyunOss> implements AliyunOssService {
    ...
    public void create(AliyunOss aliyunOss) {
        ...
        if (count > 0) { //保存
            ...
            //删除数据
            redisTemplate.delete(RedisKeyConstant.ALIYUNOSSIMPL + "AliyunOss.class");
    ...
    public AliyunOss single() {
        ...
        //首先从缓存中读取
        AliyunOss aliyunOss = (AliyunOss) redisTemplate.opsForValue().get(RedisKeyConstant.ALIYUNOSSIMPL + "AliyunOss.class");
        if (aliyunOss == null) {
            ...
            //进行数据缓存
            redisTemplate.opsForValue().set(RedisKeyConstant.ALIYUNOSSIMPL + "AliyunOss.class",aliyunOss);
        }
